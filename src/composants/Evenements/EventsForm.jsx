...
export const EventForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.heure.debut) newErrors['heure.debut'] = 'L\'heure de début est requise.';
    if (!formData.heure.fin) newErrors['heure.fin'] = 'L\'heure de fin est requise.';
    if (!formData.lieu.nom.trim()) newErrors['lieu.nom'] = 'Le nom du lieu est requis.';
    ...
    if (formData.date && formData.heure.debut && formData.heure.fin) {
        const dateTimeDebut = new Date(`${formData.date}T${formData.heure.debut}`);
        const dateTimeFin = new Date(`${formData.date}T${formData.heure.fin}`);
        if (dateTimeFin < dateTimeDebut) {
            newErrors['heure.fin'] = 'L\'heure de fin ne peut pas être antérieure à l\'heure de début.';
        }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const capaciteValue = formData.capacite === '' ? undefined : parseInt(formData.capacite, 10);
      ...
      if (!eventData.lieu.adresse.rue && !eventData.lieu.adresse.ville && !eventData.lieu.adresse.codePostal && !eventData.lieu.adresse.pays) {
        delete eventData.lieu.adresse;
      }
      if (!eventData.lieu.coordonnees.latitude && !eventData.lieu.coordonnees.longitude) {
      ...
      const fakeApiResponse = await new Promise(resolve => setTimeout(() => {
      ...
      if (fakeApiResponse.success) {
        ...
        setFormData({ // Réinitialiser le formulaire après succès
          ...
          date: '',
          heure: { debut: '', fin: '' },
          lieu: { nom: '', adresse: { rue: '', ville: '', codePostal: '', pays: '' }, coordonnees: { latitude: '', longitude: '' } },
          type: '',
          image: { url: '', alt: '' },
          capacite: '',
          estPublic: true,
        ...
        setMessage(fakeApiResponse.message || 'Échec de la création de l\'événement.');
        setIsSuccess(false);
      }

    } catch (error) {
      console.error("Erreur lors de la création de l'événement:", error);
      setMessage(error.response?.data?.message || 'Une erreur inattendue est survenue.');
      setIsSuccess(false);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-md rounded-lg my-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Créer un Nouvel Événement</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Titre */}
        <div>
          <label htmlFor="titre" className="block text-sm font-medium text-gray-700">Titre de l'événement <span className="text-red-500">*</span></label>
          <input
            type="text"
            id="titre"
            name="titre"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label htmlFor="heure.debut" className="block text-sm font-medium text-gray-700">Heure de début <span className="text-red-500">*</span></label>
            <input
              type="time"
              id="heure.debut"
              name="heure.debut"
              ...
              required
            />
            {errors['heure.debut'] && <p className="mt-1 text-sm text-red-600">{errors['heure.debut']}</p>}
          </div>
          <div>
            <label htmlFor="heure.fin" className="block text-sm font-medium text-gray-700">Heure de fin <span className="text-red-500">*</span></label>
            <input
              type="time"
              id="heure.fin"
              name="heure.fin"
              ...
              required
            />
            {errors['heure.fin'] && <p className="mt-1 text-sm text-red-600">{errors['heure.fin']}</p>}
          </div>
        </div>

        {/* Lieu (Nom du lieu) */}
        <div>
          <label htmlFor="lieu.nom" className="block text-sm font-medium text-gray-700">Nom du Lieu <span className="text-red-500">*</span></label>
          <input
            type="text"
            id="lieu.nom"
            name="lieu.nom"
            ...
            // required
          />
          {errors['lieu.nom'] && <p className="mt-1 text-sm text-red-600">{errors['lieu.nom']}</p>}
        </div>

        {/* Type d'événement */}
        <div>
          <label htmlFor="type" className="block text-sm font-medium text-gray-700">Type d'événement <span className="text-red-500">*</span></label>
          <select
            id="type"
            name="type"
            value={formData.type}
        ...
        <fieldset className="border p-4 rounded-md shadow-sm">
          <legend className="text-md font-semibold text-gray-800 mb-3">Adresse du Lieu (Optionnel)</legend>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="lieu.adresse.rue" className="block text-sm font-medium text-gray-700">Rue</label>
              <input
                type="text"
                id="lieu.adresse.rue"
            ...
            <div>
              <label htmlFor="lieu.adresse.ville" className="block text-sm font-medium text-gray-700">Ville</label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="lieu.adresse.codePostal" className="block text-sm font-medium text-gray-700">Code Postal</label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="lieu.adresse.pays" className="block text-sm font-medium text-gray-700">Pays</label>
              <input
                type="text"
        ...
        <fieldset className="border p-4 rounded-md shadow-sm">
          <legend className="text-md font-semibold text-gray-800 mb-3">Coordonnées GPS (Optionnel)</legend>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="lieu.coordonnees.latitude" className="block text-sm font-medium text-gray-700">Latitude</label>
              <input
                type="number"
            ...
            <div>
              <label htmlFor="lieu.coordonnees.longitude" className="block text-sm font-medium text-gray-700">Longitude</label>
              <input
                type="number"
        ...
        {/* Image */}
        <fieldset className="border p-4 rounded-md shadow-sm">
          <legend className="text-md font-semibold text-gray-800 mb-3">Image de l'événement (Optionnel)</legend>
          <div>
            <label htmlFor="image.url" className="block text-sm font-medium text-gray-700">URL de l'image</label>
            <input
              type="text"
              id="image.url"
              name="image.url"
          ...
          <div className="mt-4">
            <label htmlFor="image.alt" className="block text-sm font-medium text-gray-700">Texte alternatif (Alt)</label>
            <input
              type="text"
              id="image.alt"
              name="image.alt"
        ...
        {/* Capacité */}
        <div>
          <label htmlFor="capacite" className="block text-sm font-medium text-gray-700">Capacité (nombre de participants)</label>
          <input
            type="number"
            id="capacite"
            name="capacite"
        ...
        <div className="flex items-center">
          ...
          />
          <label htmlFor="estPublic" className="ml-2 block text-sm text-gray-900">
